...
// Get-Process command
export async function getProcess(args: string[], session: TerminalSession): Promise<ExecutionResult> {
...
export async function stopProcess(args: string[], session: TerminalSession): Promise<ExecutionResult> {
  if (args.length === 0) {
    return {
