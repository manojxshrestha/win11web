...
class FilesystemManager {
    constructor(dbPath = './data/filesystem.db') {
      ...
      `);
            const insertMany = this.db.transaction((dirs) => {
                for (const dir of dirs) {
                    const name = dir === 'C:\\' ? 'C:' : path_1.default.basename(dir);
                    insert.run(name, dir);
                }
            });
    ...
    moveFile(sourcePath, destPath) {
        ...
        const newName = path_1.default.basename(normalizedDest);
    ...
    getShares() {
        return this.db.prepare('SELECT name, path, description FROM shares').all();
    }
    deleteShare(name) {
