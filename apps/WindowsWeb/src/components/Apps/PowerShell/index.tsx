...
export function PowerShell() {
  const [tabs, setTabs] = useState<PowerShellTab[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      socket.on('connect', () => {
        console.log('PowerShell: Connected to terminal server');
      });
      ...
      socket.on('terminal-ready', () => {
        console.log('PowerShell: Terminal ready');
      });

      socket.on('terminal-error', (error: { message: string }) => {
        console.error('PowerShell: Terminal error:', error);
      });
  ...
  const createNewTab = async (socket?: Socket) => {
    const sessionId = `powershell-${Date.now()}`;
    ...
    setTimeout(() => {
      ...
      if (container) {
        ...
        if (s && s.connected) {
          s.emit('create-terminal-session', { sessionId, shell: 'powershell' });
        } else if (s) {
          s.once('connect', () => {
            s.emit('create-terminal-session', { sessionId, shell: 'powershell' });
          });
  ...
  const handleTabCompletion = async (terminal: XTermType, sessionId: string, currentInput: string) => {
    ...
    if (completions.length === 1) {
      ...
      if (socketRef.current && socketRef.current.connected) {
        setTimeout(() => {
          socketRef.current?.emit('create-terminal-session', { sessionId, shell: 'powershell' });
        }, 50);
      }
    }
  };
  ...
  return (
    <Box height="100%" display="flex" flexDirection="column" bg="#012456">
      {/* Tab bar */}
      <HStack
        spacing={0}
        bg="#1a1a1a"
