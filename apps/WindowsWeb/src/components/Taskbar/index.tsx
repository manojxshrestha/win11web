...
import { Discover, TaskbarContextMenu, TaskbarIcon } from '@repo/ui/components';
import {
						...
						<HStack spacing={1} ml={2}>
									...
									<TaskbarIcon
										...
										onClick={
											windows[app.processName]
												? handleShowWindow(
														app.processName,
														getKeys(windows[app.processName])?.[0] ?? 0
												  )
											: handleAddWindow(app)
										}
									/>
							...
							{runningProcesses
								...
								.map(([process, processWindows]) => (
									...
									<TaskbarIcon
										app={getValues(processWindows)?.[0] ?? { shortName: process, icon: null }}
										key={process}
										onClick={handleShowWindow(
											process,
											getKeys(processWindows)?.[0] ?? 0
										)}
									/>
									</WindowsPreview>
								))}
